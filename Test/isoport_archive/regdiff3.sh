...
norm() { awk -v T="$2" 'index($0, T".thy") {on=1} on' "$1" \
  ...
  | grep -v '^Output (line' | grep -v '^Warning (line' | grep -v '^$' \
  | grep -v 'cached proof fails' | grep -v 'Proof cache for theory' \
  | grep -v 'Re-searching proofs' \
  | sort }
for T in $@; do
  A=$SP/reg_${TA}_$T.txt; B=$SP/reg_${TB}_$T.txt
  if [ ! -f $A ] || [ ! -f $B ]; then echo "$T : MISSING"; continue; fi
  if diff -q <(norm $A $T) <(norm $B $T) > /dev/null; then echo "$T : IDENTICAL (content, order-insensitive)"
  else echo "$T : CONTENT DIFFERS"; diff <(norm $A $T) <(norm $B $T) | head -30; fi
done
