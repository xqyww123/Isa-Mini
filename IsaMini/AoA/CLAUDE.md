...
- **Tools** (abstract ids in `model.py`): `edit` (TOOL_EDIT), `delete`, `query` (TOOL_SEARCH), `recall` (TOOL_READ), `refute_or_surrender`, `request_lemmas` (dual-role: worker→planner channel, or planner self-formalize hint), answer-*. Tool→op mapping in `mcp_http_server.py` (`_edit_tool_logic` → `root.fill` / `insert_before` / `amend`; `_delete_tool_logic` → `root.delete`). Schemas are JSONC in `tools/`.
