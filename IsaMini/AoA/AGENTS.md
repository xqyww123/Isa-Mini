...
- **Role ADT** (stageless): `Role_Major` (the single continuous main/planner agent — runs real proofs throughout; there is **no** PLANNING/FINISHING stage), `Role_Worker(target, suggestions, useful_lemmas, worker_handle)`, `Role_Interaction(...)` (`model.py`, all `@dataclass`). Predicates `Session.is_major` / `is_worker` / `is_interaction` (all `@property` — no parens; there is **no** `is_planning`).
- **`Runtime`**: shared singleton — `age`, depth limit (30), `total_tool_calls`, `worker_max_tool_calls` (500).
- **Worker dispatch is on-demand, NOT a staged sweep.** There is no `complete_proof`, no FINISHING re-evaluation, and no `_collect_failed_obvious_parents` (those names exist only in stale docs — zero hits in code). Instead the main agent reads a rendered failed `Obvious` (plus its `_print_subagent_hint` for deep ones) and *chooses* to delegate it by calling the `subagent` tool (`_subagent_tool_logic` in `mcp_http_server.py`), which redirects to the target's nearest delegatable goal (`_nearest_goal_for_subagent`; refuses the whole-goal case, see `mcp_http_server.py` `target.parent is psr`) and forks a worker. `WorkerHandle` (defined in **`model.py`**, attached as `node.worker_handle`) owns the worker's asyncio task + event queue; `run_until_yield` drives it until it yields a terminal outcome or a park. Events (everything queued on the handle): `WorkerRefute` / `WorkerSurrender` / `WorkerRegionDead` / `WorkerRequestConstraints` / `WorkerDifficulty` / `WorkerDone` (synthesised on task completion). NOT events: `run_until_yield` *returns* a `WorkerYield`, and wraps an accepted refute into a `WorkerComplaint` as the payload for `Interaction_ReviewRefutation`. The `request` tool (constant `TOOL_REQUEST_LEMMAS`) is a worker→planner channel: the worker BLOCKS while the **planner itself** authors + proves the accepted helper lemmas into the global env, then the worker resumes with them in scope (no separate lemma sub-agent / `LemmaDrive`). Workers may themselves dispatch nested sub-agents. Worker success = `target.is_proof_finished()` (NOT status!).
...
- Helpers in `model.py`: `print_vars` (banner `variables`), `print_hyps` (banner **`premises`**), `print_goal`, `print_pending_goal` (banner **`pending proof goal:`**). The worker view (`print_proof_scope`) folds declared facts into that same **`premises:`** banner.
...
- **Dump state** for the golden: `root.print(0,file)`, `root.quickview(0,file)`, `print_header(msg,file)`, or `file.write(...)`. For **worker scoped views**: `session = root.session; session.role = model.Role_Worker(target=have_node)` then `session.print_proof_scope(0,file)`; restore with `session.role = model.Role_Major()`.
